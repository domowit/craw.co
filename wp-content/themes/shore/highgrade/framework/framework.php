...
if( !class_exists( 'ReduxFramework' ) ) {
    ...
    class ReduxFramework {
        ...
        public $framework_url       = 'http://highgradelab.com/';
        ...
        public $args                = array(
            'opt_name'           => 'redux_options',             // Must be defined by theme/plugin
            ...
            'save_defaults'      => false,           // Save defaults to the DB on it if empty
            'footer_credit'      => '',
            ...
            'global_variable'    => 'hgr_options',             // Changes global variable from $GLOBALS['YOUR_OPT_NAME'] to whatever you set here. false disables the global variable
            'output'             => true,           // Dynamically generate CSS
            'compiler'           => true,           // Initiate the compiler hook
            'output_tag'         => true,           // Print Output Tag
            'transient_time'     => '',
            'default_show'       => true,          // If true, it shows the default value
        ...
        public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
            ...
            if ( empty( $this->args['footer_credit'] ) ) {
                $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Powered by %1$s', 'redux-framework' ), '<a href="'.esc_url( $this->framework_url ).'" target="_blank">'.__( 'HighGrade Framework', 'redux-framework' ).'</a> v'.self::$_version ) . '</span>';
            }
...
//                $comparison = $field['required'][1];
            ...
            if ( !empty( $this->typography ) && !empty( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                ...
                <?php
                   ...
                   // echo '<link rel="stylesheet" id="options-google-fonts"  href="'.$typography->makeGoogleWebfontLink( $this->typography ).'&amp;v='.$version.'" type="text/css" media="all" />';
                    ...
                    echo '<link rel="stylesheet" id="options-google-fonts" title="hgr-google-fonts" href="'.$protocol.$typography->makeGoogleWebfontLink( $this->typography ).'&amp;v='.$version.'" type="text/css" media="all" />';
        ...
        public function _options_page_html() {
            ...
            echo '<img src="'.$this->args['theme_logo'].'" class="themeLogo">';
